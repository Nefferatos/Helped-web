...
export const listMaidRecordsSql = async (filters: {
  search?: string
  visibility?: string
  agencyId?: number
}) => {
...
export const updateMaidMediaSql = async (
  ...
  const current = await getMaidByReferenceCodeSql(referenceCode, { agencyId })
  if (!current) return null

  const next = {
    ...current,
    ...(patch.photoDataUrls !== undefined ? { photoDataUrls: patch.photoDataUrls } : {}),
    ...(patch.photoDataUrl !== undefined ? { photoDataUrl: patch.photoDataUrl } : {}),
    ...(patch.videoDataUrl !== undefined ? { videoDataUrl: patch.videoDataUrl } : {}),
    ...(patch.hasPhoto !== undefined ? { hasPhoto: patch.hasPhoto } : {}),
  }
  ...
  const result = (await query(
    ...
    [
      referenceCode,
      agencyId,
      JSON.stringify(next.photoDataUrls ?? []),
      next.photoDataUrl ?? '',
      next.videoDataUrl ?? '',
      next.hasPhoto ?? false,
    ]
...
export const upsertMaidRecordsSql = async (
  ...
  try {
    ...
    for (const record of records) {
      const existing = await client.query<MaidRow>(
        `
          SELECT *
          FROM maids
          WHERE reference_code = $1 AND agency_id = $2
      ...
      if (existing.rows[0]) {
        await client.query(
          ...
          `,
          [
            existing.rows[0].reference_code,
            agencyId,
            record.fullName,
            ...
            record.homeAddress,
            record.airportRepatriation,
            record.educationLevel,
            JSON.stringify(record.languageSkills ?? {}),
            JSON.stringify(record.skillsPreferences ?? {}),
            JSON.stringify(record.workAreas ?? {}),
            JSON.stringify(record.employmentHistory ?? []),
            JSON.stringify(record.introduction ?? {}),
            JSON.stringify(record.agencyContact ?? {}),
            JSON.stringify(record.photoDataUrls ?? []),
            record.photoDataUrl ?? '',
            record.videoDataUrl ?? '',
