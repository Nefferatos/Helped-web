...
const READY_TO_POST_STAGE: RecruitmentStage =
  "Ready to Configure Public Profile";
...
const normalizeRecruitmentStage = (
  value: unknown,
  ...
  try {
    ...
    const normalized = {
      ...
      applications: (parsed.applications ?? []).map((application) => ({
        ...application,
        status:
          normalizeRecruitmentStage(application.status) ?? "New Applicant",
      })),
      ...
      history: (parsed.history ?? []).map((record) => ({
        ...
        fromStage: record.fromStage
          ? normalizeRecruitmentStage(record.fromStage)
          : undefined,
...
const buildApplicationCode = () =>
  `APP-${randomUUID().slice(0, 8).toUpperCase()}`;
...
const inferCookingSkills = (maid: MaidRecord) => {
  ...
  if (
    maid.workAreas &&
    Object.prototype.hasOwnProperty.call(maid.workAreas, "Cooking")
  ) {
...
const inferExperienceYears = (maid: MaidRecord) => {
  const jobs = Array.isArray(maid.employmentHistory)
    ? maid.employmentHistory
    : [];
  return Math.max(
    jobs.length,
    jobs.reduce((sum, item) => {
      const maybeYears = Number(
        (item as Record<string, unknown>).yearsOfExperience ?? 0,
      );
...
const inferCountriesWorked = (maid: MaidRecord) =>
  normalizeStringArray(
    (Array.isArray(maid.employmentHistory) ? maid.employmentHistory : []).map(
      (item) =>
        (item as Record<string, unknown>).country ??
...
const profileFromMaid = (
  ...
  const certifications = normalizeStringArray([
    ...(Array.isArray(introduction.certifications)
      ? introduction.certifications
      : []),
    ...(Array.isArray(introduction.trainingRecords)
      ? introduction.trainingRecords
      : []),
  ]);
  const availableDate = String(introduction.availability || "").trim();
  const expectedSalary = Number(
    introduction.expectedSalary ?? introduction.presentSalary ?? NaN,
  );
  return {
    id: randomUUID(),
    applicationId,
    ...
    email: "",
    whatsappNumber: String(
      agencyContact.phone || agencyContact.homeCountryContactNumber || "",
    ),
    ...
    petCareExperience: Number(
      (
        (maid.skillsPreferences as Record<string, unknown>)
          ?.otherInformation as Record<string, unknown>
    ...
    medicalStatus: String(introduction.otherIllnesses || "").trim()
      ? "pending"
      : "passed",
    passportStatus: String(agencyContact.passportNo || "").trim()
      ? "submitted"
      : "missing",
    certificationStatus: certifications.length > 0 ? "submitted" : "missing",
    introductionVideoUrl: maid.videoDataUrl || "",
    workHistory: Array.isArray(maid.employmentHistory)
      ? maid.employmentHistory
      : [],
    fdwFormData: {},
    strengthsTags: [],
...
const computeScore = (
  ...
  const referencesFactor = clamp(
    references.length > 0
      ? (references.reduce((sum, item) => sum + item.rating, 0) /
          (references.length * 5)) *
          100
      : 20,
  );
  ...
  const strengths = [
    ...
    certificationsFactor >= 50
      ? "Relevant certifications/training available"
      : "",
  ...
  const weaknesses = [
    ...
    interview && interview.interviewResult === "Failed"
...
const createDefaultBackgroundCheck = (
  applicationId: string,
): MaidBackgroundCheckRecord => ({
  id: randomUUID(),
  applicationId,
...
const createPublicDocuments = (
  uploaded: Array<
    Pick<MaidApplicationDocumentRecord, "type" | "name" | "url" | "uploadedAt">
  >,
): MaidApplicationDocumentRecord[] => {
  const defaults: Array<
    Pick<MaidApplicationDocumentRecord, "type" | "name" | "required">
  > = [
...
const persistPublicApplicantFile = async (
  ...
  const relativePath = path.posix.join(
    "ats-applicants",
    `agency-${agencyId}`,
    sanitizePathSegment(applicationId, "application"),
    kind,
    fileName,
  );
...
const meetsDocumentsRequirement = (docs: MaidApplicationDocumentRecord[]) =>
  docs.every(
    (doc) =>
      !doc.required || doc.status === "submitted" || doc.status === "verified",
  );
  ...
  if (!meetsDocumentsRequirement(docs)) {
    ...
    if (application.status !== "Rejected") {
      transition(
        ...
        "Rejected",
        actor,
        "Auto reject: missing required documents",
      );
    }
    return;
  }
  if (profile.medicalStatus === "failed") {
    ...
    if (application.status !== "Rejected") {
      transition(
        ...
        "Rejected",
        actor,
        "Auto reject: failed medical check",
      );
    }
    return;
  }
  if (
    backgroundCheck.identityVerified === false &&
    backgroundCheck.result === "Failed"
  ) {
    ...
    if (application.status !== "Rejected") {
      transition(
        ...
        "Rejected",
        actor,
        "Auto reject: failed identity verification",
      );
    }
    return;
  }
  if (profile.yearsOfExperience < 1) {
    ...
    if (application.status !== "Rejected") {
      transition(
        ...
        "Rejected",
        actor,
        "Auto reject: minimum experience not met",
      );
    }
    return;
  }
...
export const ensureApplicationForMaid = async (maid: MaidRecord) => {
  ...
  const existing = data.applications.find(
    (item) =>
      item.agencyId === maid.agencyId &&
      item.maidReferenceCode === maid.referenceCode,
  );
...
export const createPublicAtsApplication = async (
  payload: PublicApplicantSubmissionInput,
) => {
  const data = await readData();
  const applicationId = randomUUID();
  const createdAt = now();
  ...
  for (const file of payload.files) {
    try {
      const url = await persistPublicApplicantFile(
        payload.agencyId,
        applicationId,
        file,
      );
  ...
  const profile: MaidProfileRecord = {
    // try to pick up a reference code provided in FDW form data (common keys: referenceCode, reference, maidReferenceCode)
    maidReferenceCode: (() => {
      try {
        const raw = (payload.fdwFormData as Record<string, unknown>) || {};
        return (
          String(
            raw.referenceCode ?? raw.reference ?? raw.maidReferenceCode ?? "",
          ).trim() || undefined
        );
      } catch {
        return undefined;
    ...
    medicalStatus: uploadedDocs.some((item) => item.type === "medical")
      ? "pending"
      : "pending",
    passportStatus: uploadedDocs.some((item) => item.type === "passport")
      ? "submitted"
      : "missing",
    certificationStatus: uploadedDocs.some(
      (item) => item.type === "certificate",
    )
      ? "submitted"
      : "missing",
    introductionVideoUrl: uploadedDocs.find((item) => item.type === "video")
      ?.url,
    ...
    weaknessesTags: normalizeStringArray([
      payload.files.some((item) => item.kind === "passport")
        ? ""
        : "Passport missing",
      payload.files.some((item) => item.kind === "certificate")
        ? ""
  ...
  const application: MaidApplicationRecord = {
    ...
    profileId: profile.id,
    // carry forward any supplied maid reference code to the application record
  ...
  const notifications: MaidNotificationRecord[] = [
    ...
    buildNotification(
      ...
      "internal",
      `${payload.fullName} submitted a new applicant portal form.`,
    ),
  ];
...
const getApplicationBundleInternal = (
  data: AtsData,
  application: MaidApplicationRecord,
) => {
  const profile =
    data.profiles.find((item) => item.applicationId === application.id) ?? null;
  const score =
    data.scores.find((item) => item.applicationId === application.id) ?? null;
  const interview =
    data.interviews.find((item) => item.applicationId === application.id) ??
    null;
  const backgroundCheck =
    data.backgroundChecks.find(
...
export const getAtsApplication = async (
  agencyId: number,
  applicationId: string,
) => {
  const data = await readData();
...
export const getPublicAtsApplicationSummary = async (
  applicationId: string,
  accessToken: string,
) => {
  const data = await readData();
...
export const listAtsApplications = async (
  ...
  const profilesByApplication = new Map(
    data.profiles.map((profile) => [profile.applicationId, profile]),
  );
  const scoresByApplication = new Map(
    data.scores.map((score) => [score.applicationId, score]),
  );
  const interviewsByApplication = new Map(
    data.interviews.map((interview) => [interview.applicationId, interview]),
  );
  const matchesByApplication = new Map<string, number>();
  data.matches.forEach((match) => {
    const current = matchesByApplication.get(match.applicationId) ?? 0;
    if (match.compatibilityScore > current)
      matchesByApplication.set(match.applicationId, match.compatibilityScore);
  });

  let items = data.applications.filter(
    (application) => application.agencyId === agencyId,
  );
  ...
  items = items.filter((application) => {
    ...
    if (nationality?.length && !nationality.includes(profile.nationality))
      return false;
    ...
    if (
      languages?.length &&
      !languages.some((item) => profile.languageSkills.includes(item))
    )
      return false;
    ...
    if (
      ...
      profile.yearsOfExperience < minExperience
    )
      return false;
    ...
    if (Number.isFinite(salaryMax) && (profile.expectedSalary ?? 0) > salaryMax)
      return false;
    ...
    if (
      hasCooking?.length &&
      !hasCooking.some((item) => profile.cookingSkills.includes(item))
    )
      return false;

    const medicalStatus = filters.medicalStatus as string[] | undefined;
    if (medicalStatus?.length && !medicalStatus.includes(profile.medicalStatus))
      return false;

    const passportStatus = filters.passportStatus as string[] | undefined;
    if (
      passportStatus?.length &&
      !passportStatus.includes(profile.passportStatus)
    )
      return false;

    const certificationStatus = filters.certificationStatus as
      | string[]
      | undefined;
    if (
      certificationStatus?.length &&
      !certificationStatus.includes(profile.certificationStatus)
    )
      return false;
  ...
  items.sort((left, right) => {
    ...
    switch (sort) {
      ...
      case "availability:asc":
        return String(leftProfile.availableDate).localeCompare(
          String(rightProfile.availableDate),
        );
      ...
      case "referenceRating:desc":
        return (
          getReferencesForProfile(rightProfile).reduce(
            (sum, item) => sum + item.rating,
            0,
          ) -
          getReferencesForProfile(leftProfile).reduce(
            (sum, item) => sum + item.rating,
            0,
          )
        );
      case "numberOfLanguages:desc":
        return (
          rightProfile.languageSkills.length - leftProfile.languageSkills.length
        );
      case "clientMatchScore:desc":
        return rightMatch - leftMatch;
      case "expectedSalary:asc":
        return (
          (leftProfile.expectedSalary ?? Number.MAX_SAFE_INTEGER) -
          (rightProfile.expectedSalary ?? Number.MAX_SAFE_INTEGER)
        );
      case "name:asc":
...
export const upsertInterview = async (
  ...
  payload: Omit<
    MaidInterviewRecord,
    "id" | "applicationId" | "createdAt" | "updatedAt" | "score"
  >,
) => {
  const data = await readData();
  const application = data.applications.find(
  ...
  const existingIndex = data.interviews.findIndex(
    (item) => item.applicationId === applicationId,
  );
  const score = Math.round(
  ...
  const interview: MaidInterviewRecord = {
    ...
    score,
    createdAt:
      existingIndex === -1 ? now() : data.interviews[existingIndex].createdAt,
    updatedAt: now(),
  };
  ...
  if (profile) {
    ...
    const background =
      data.backgroundChecks.find(
        (item) => item.applicationId === applicationId,
      ) ?? createDefaultBackgroundCheck(applicationId);
    applyAutomationRules(
      data,
      application,
      profile,
...
export const upsertBackgroundCheck = async (
  ...
  payload: Omit<
    MaidBackgroundCheckRecord,
    "id" | "applicationId" | "completionPercentage" | "updatedAt"
  >,
) => {
  const data = await readData();
  const application = data.applications.find(
  ...
  if (!application) throw new Error("APPLICATION_NOT_FOUND");
  const existingIndex = data.backgroundChecks.findIndex(
    (item) => item.applicationId === applicationId,
  );
  const checklist = [
  ...
  const record: MaidBackgroundCheckRecord = {
    id:
      existingIndex === -1
        ? randomUUID()
        : data.backgroundChecks[existingIndex].id,
    applicationId,
    ...payload,
  ...
  const interview =
    data.interviews.find((item) => item.applicationId === applicationId) ??
    null;
  if (profile && score) {
    transition(
      data,
  ...
  if (profile) {
    ...
    data.notifications.unshift(
      buildNotification(
        ...
        profile.contactNumber,
        "internal",
        `${profile.fullName} moved to ${nextStage}`,
      ),
    );
  }
  await writeData(data);
...
export const matchApplicationsToRequirement = async (
  ...
  const ranked = applications
    .map((application) => {
      ...
      if (
        requirementIncludes(requirement, [
          "indonesian",
          "filipino",
          "myanmar",
          "indian",
        ...
        if (
          requirement.includes(profile.nationality.toLowerCase().split(" ")[0])
        ) {
          compatibility += 15;
      ...
      if (requirementIncludes(requirement, ["chinese food", "chinese"])) {
        if (
          profile.cookingSkills.some((item) =>
            item.toLowerCase().includes("chinese"),
          )
        ) {
          compatibility += 8;
      ...
      if (requirementIncludes(requirement, ["english"])) {
        if (
          profile.languageSkills.some((item) =>
            item.toLowerCase().includes("english"),
          )
        ) {
          compatibility += 6;
      ...
      if (budgetMatch && profile.expectedSalary != null) {
        ...
        if (profile.expectedSalary <= budget) {
          compatibility += 8;
          skillsMatched.push(
            `Salary within budget (${profile.expectedSalary})`,
          );
        } else {
          missingRequirements.push(
            `Salary above budget (${profile.expectedSalary})`,
          );
        }
      }
      if (requirementIncludes(requirement, ["immediately", "immediate"])) {
        if (
          ...
          new Date(profile.availableDate).getTime() <= Date.now() + 7 * 86400000
        ) {
          compatibility += 5;
  ...
  return ranked.map((item) => ({
    ...
    qualificationScore:
      data.scores.find((score) => score.applicationId === item!.application.id)
        ?.score ?? 0,
...
export const getAtsDashboard = async (agencyId: number) => {
  ...
  return {
    ...
    newApplicants: items.filter((item) => item.status === "New Applicant")
      .length,
    interviewedCandidates: items.filter(
      (item) =>
        stageOrder.indexOf(item.status) >=
        stageOrder.indexOf("Screening Interview"),
    ).length,
    approvedCandidates: approved,
    rejectedCandidates: items.filter((item) => item.status === "Rejected")
      .length,
    readyForMatching: items.filter(
      (item) => item.status === READY_TO_POST_STAGE,
    ).length,
    ...
    averageTimeToApprovalDays:
      approvalDurations.length > 0
        ? Math.round(
            approvalDurations.reduce((sum, item) => sum + item, 0) /
              approvalDurations.length /
              86400000,
          )
        : 0,
    placementSuccessRate:
      approved > 0 ? Math.round((placed / approved) * 100) : 0,
...
export const getAtsFilterPresets = async (agencyId: number) => {
  ...
  const defaults = [
    {
      id: "preset-childcare",
      agencyId,
      name: "Experienced Childcare Helpers",
      filters: { childcareExperience: true, minExperience: 3 },
      createdAt: now(),
    },
    {
      id: "preset-newborn",
      agencyId,
      name: "Newborn Specialists",
      filters: { newbornCareExperience: true },
      createdAt: now(),
    },
    {
      id: "preset-elderly",
      ...
      filters: { elderlyCareExperience: true },
      createdAt: now(),
    },
    {
      id: "preset-cooking",
      agencyId,
      name: "Cooking Experts",
      filters: { cookingSkills: ["Chinese food"] },
      createdAt: now(),
    },
    {
      id: "preset-ready",
      ...
      filters: { status: [READY_TO_POST_STAGE, "Approved"] },
      createdAt: now(),
    },
    {
      id: "preset-approved",
      agencyId,
      name: "Approved Helpers",
  ...
  return [
    ...defaults,
    ...data.savedFilters.filter((item) => item.agencyId === agencyId),
  ];
};
