...
import { clearAgencyAdminAuth, getAgencyAdminToken, saveAgencyAdminAuth } from "@/lib/agencyAdminAuth";
...
const AgencyAdminLogin = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isLogin ? "/api/agency-auth/login" : "/api/agency-auth/register";
      ...
      toast.success(isLogin ? "Agency admin logged in" : "Agency admin account created");
      navigate(adminPath("/dashboard"));
    } catch (error) {
        ...
        <div className="rounded-2xl bg-card p-8 shadow-lg">
          <div className="mb-8 text-center">
            ...
            <p className="font-body text-sm text-muted-foreground">
              {isLogin
                ? "Main agency can use username `admin` and password `admin123`."
          ...
          <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4">
            {!isLogin ? (
              <div>
                <label className="mb-1 block font-body text-xs uppercase tracking-wider text-muted-foreground">Agency Name</label>
                <input
                  type="text"
                  value={agencyName}
            ...
            <div>
              <label className="mb-1 block font-body text-xs uppercase tracking-wider text-muted-foreground">Username</label>
              <input
                type="text"
                value={username}
            ...
            <Button type="submit" size="lg" className="w-full font-body" disabled={isSubmitting}>
              {isSubmitting ? "Please wait..." : isLogin ? "Sign In to Agency Portal" : "Create Agency Portal"}
            </Button>
          </form>

          <div className="mt-6 text-center">
            <p className="font-body text-sm text-muted-foreground">
              {isLogin ? "Need a new agency portal?" : "Already have an agency portal?"}{" "}
