...
const ClientLandingPage = ({ embedded = false }: ClientLandingPageProps) => {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      try { const c = await syncClientProfileFromSession(); if (c) setClientUser(c); } catch { /* ignore session sync errors */ }
    };
    void sync();
  }, [isLoggedIn]);
  ...
  return (
    <div className="int client-landing" style={{ position:"relative", minHeight: "100vh", background: "#fff", fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        ...
        {showSocialRail && [
          { label: "Facebook", href: "https://www.facebook.com/share/1Bi1dLHTQw/", color: "#1877F2", icon: <Facebook size={20} /> },
          { label: "TikTok", href: "https://www.tiktok.com/@atagency.maid.sol?_r=1&_t=ZS-98H4zr4nZ4s", color: "#111111", icon: <FaTiktok size={19} /> },
          { label: "YouTube", href: "https://www.youtube.com/@atagencymaidsolutions?si=lNyfLWv6k6dcsKSK", color: "#FF0000", icon: <Youtube size={20} /> },
        ].map((social) => (
          <a
            key={social.label}
            href={social.href}
