...
const navItems = [
  ...
  {
    label: "AI HR Interviewer",
    path: adminPath("/ai-hr-interviewer"),
    icon: Brain,
    iconBg: "linear-gradient(145deg, #C4B5FD, #7C3AED)",
    iconShadow: "0 4px 0 #6D28D9, 0 6px 12px rgba(124,58,237,0.45)",
    iconShadowActive: "0 2px 0 #6D28D9, 0 3px 8px rgba(124,58,237,0.4)",
...
/**
 * Derive a per-agency localStorage key so that different agency admins on the
 * same browser never bleed notification state into each other.
 * Falls back to the legacy global key when no agencyId is available.
 */
const getNotificationCacheKey = (agencyId?: number | null) =>
  agencyId && agencyId > 0
    ? `helped-admin-notification-cache-a${agencyId}`
    : "helped-admin-notification-cache";
...
const readAdminNotificationCache = (agencyId?: number | null): AdminNotificationCache | null => {
  if (typeof window === "undefined") return null;

  try {
    // Try agency-scoped key first, fall back to legacy global key.
    const scopedKey = getNotificationCacheKey(agencyId);
    const raw =
      window.localStorage.getItem(scopedKey) ??
      (agencyId ? window.localStorage.getItem(getNotificationCacheKey()) : null);
    if (!raw) return null;
...
const writeAdminNotificationCache = (
  agencyId: number | undefined | null,
  cache: AdminNotificationCache,
) => {
  if (typeof window === "undefined") return;

  try {
    window.localStorage.setItem(getNotificationCacheKey(agencyId), JSON.stringify(cache));
    // Remove the legacy global key so it doesn't leak to other agency sessions.
    if (agencyId) {
      window.localStorage.removeItem(getNotificationCacheKey());
    }
  } catch {
...
const clearNotificationCacheForAgency = (agencyId?: number | null) => {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(getNotificationCacheKey(agencyId));
    window.localStorage.removeItem(getNotificationCacheKey());
  } catch { /* ignore */ }
};
...
const AppLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const agencyId = getStoredAgencyAdmin()?.agencyId ?? null;
  const [badgeCounts, setBadgeCounts] = useState<BadgeCounts>(
    () => readAdminNotificationCache(agencyId)?.badgeCounts ?? EMPTY_BADGE_COUNTS
  );
  const [chatNotifications, setChatNotifications] = useState<SupportNotification[]>(
    () => readAdminNotificationCache(agencyId)?.chatNotifications ?? []
  );
  const [agencyAdmin, setAgencyAdmin] = useState<AgencyAdminUser | null>(
    getStoredAgencyAdmin()
  );
  const [agencyLogoUrl, setAgencyLogoUrl] = useState(
    () => readAdminNotificationCache(agencyId)?.agencyLogoUrl ?? ""
  );

  // ── Refs so polling can read the latest state without stale closures ────
  const badgeCountsRef = useRef(badgeCounts);
  const chatNotificationsRef = useRef(chatNotifications);
  const agencyLogoUrlRef = useRef(agencyLogoUrl);
  const agencyAdminRef = useRef(agencyAdmin);
  useEffect(() => { badgeCountsRef.current = badgeCounts; }, [badgeCounts]);
  useEffect(() => { chatNotificationsRef.current = chatNotifications; }, [chatNotifications]);
  useEffect(() => { agencyLogoUrlRef.current = agencyLogoUrl; }, [agencyLogoUrl]);
  useEffect(() => { agencyAdminRef.current = agencyAdmin; }, [agencyAdmin]);

  // ── Persist notification state to localStorage on every change ───────────
  // This is the primary fix: optimistic clears (opening bell, navigating to
  // a page) must be written to cache so that polling fallback values and
  // page-reload initialisation never restore stale counts.
  useEffect(() => {
    const currentAgencyId = agencyAdminRef.current?.agencyId;
    if (!currentAgencyId) return;
    writeAdminNotificationCache(currentAgencyId, {
      badgeCounts,
      chatNotifications,
      agencyLogoUrl,
    });
  }, [badgeCounts, chatNotifications, agencyLogoUrl]);
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      try {
        const currentAgencyId = agencyAdminRef.current?.agencyId;
        const cached = readAdminNotificationCache(currentAgencyId);
        let summaryData: {
        ...
        let companyLogo = cached?.agencyLogoUrl ?? agencyLogoUrlRef.current;
        // Use current state (not cache) as fallback – this ensures that an
        // already-acknowledged count stays at 0 even if the cache is stale.
        let unreadChats = badgeCountsRef.current.unreadChats;
        let unreadApplicants = badgeCountsRef.current.unreadApplicants;
        let dedicatedNotifications: SupportNotification[] = chatNotificationsRef.current;
        ...
        try {
          ...
          // Keep the current state during transient network hiccups.
        }
        ...
        // Build the next counts.  When the API returns a value we always use
        // it (the backend is the source of truth).  When an API call failed we
        // keep the *current* React state – which was already acknowledged –
        // rather than falling back to the (potentially stale) cache.
  ...
  const handleLogout = async () => {
    try {
      ...
      clearNotificationCacheForAgency(agencyAdminRef.current?.agencyId);
      clearAgencyAdminAuth();
  ...
  const handleNotificationsOpenChange = useCallback((open: boolean) => {
    if (!open || chatNotificationsRef.current.length === 0) return;

    // Optimistically clear chat notifications.  The useEffect that syncs
    // badgeCounts/chatNotifications to localStorage will fire immediately
    // after these state updates, ensuring the cleared state is persisted
    // before the next polling cycle can read stale cache values.
            ...
            <DropdownMenu onOpenChange={handleNotificationsOpenChange}>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className="focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0D6E56] focus-visible:ring-offset-1"
                  style={{
                    width: 38,
                    height: 38,
                    display: "flex",
                    ...
                    justifyContent: "center",
                    borderRadius: 10,
                    border: visibleUnread > 0 ? "1px solid #FCA5A5" : "1px solid #E5E7EB",
                    background: visibleUnread > 0 ? "#FEF2F2" : "#F9FAFB",
                    cursor: "pointer",
                    color: visibleUnread > 0 ? "#DC2626" : "#6B7280",
                  ...
                  onMouseEnter={(e) => {
                    ...
                    el.style.background = visibleUnread > 0 ? "#FEE2E2" : "#F3F4F6";
                    el.style.borderColor = visibleUnread > 0 ? "#F87171" : "#D1D5DB";
                  }}
                  onMouseLeave={(e) => {
                    const el = e.currentTarget as HTMLButtonElement;
                    el.style.background = visibleUnread > 0 ? "#FEF2F2" : "#F9FAFB";
                    el.style.borderColor = visibleUnread > 0 ? "#FCA5A5" : "#E5E7EB";
                  }}
                >
              ...
              <DropdownMenuContent
                ...
                style={{ maxHeight: "min(480px, calc(100vh - 80px))", overflowY: "auto" }}
              >
                    ...
                    >
                      <div className="flex min-w-0 items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-foreground">Requests need review</div>
                  ...
                  <DropdownMenuItem asChild>
                    ...
                    >
                      <div className="flex min-w-0 items-start gap-2.5">
                        ...
                        >
                          <PhoneIncoming style={{ width: 14, height: 14 }} />
                        </span>
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-foreground">Enquiries need follow-up</div>
                          <div className="text-xs text-muted-foreground">
                  ...
                  <DropdownMenuItem asChild>
                    ...
                    >
                      <div className="flex min-w-0 items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-foreground">Applicants need review</div>
                          <div className="text-xs text-muted-foreground">
                ...
                {totalUnread === 0 && visibleChatNotifications.length === 0 && (
                  <div className="flex flex-col items-center gap-2 px-3 py-8 text-center">
                    <CheckCircle style={{ width: 28, height: 28, color: "#D1D5DB" }} />
                ...
                {visibleChatNotifications.length > 0 && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem asChild>
                      <Link to={adminPath("/chat-support")} className="justify-center py-2 font-medium text-primary">
                        Open support inbox
                      </Link>
                    </DropdownMenuItem>
                  </>
