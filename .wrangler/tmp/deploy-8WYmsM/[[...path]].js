...
var normalizeMaid = /* @__PURE__ */ __name((maid) => {
  const photos = Array.isArray(maid.photoDataUrls) ? maid.photoDataUrls.filter((item) => typeof item === "string" && item.trim()) : maid.photoDataUrl ? [maid.photoDataUrl] : [];
  return {
    ...maid,
...
var mergeAppData = /* @__PURE__ */ __name((raw2) => {
  ...
  const clients = raw2.clients ?? defaults.clients;
  const agencyAdmins = raw2.agencyAdmins ?? defaults.agencyAdmins;
  ...
  return {
    companyProfile: {
      ...
      ...raw2.companyProfile,
      gallery_image_data_urls: Array.isArray(raw2.companyProfile?.gallery_image_data_urls) ? raw2.companyProfile.gallery_image_data_urls : defaults.companyProfile.gallery_image_data_urls
    },
    ...
    counters: {
      momPersonnel: nextCounter(raw2.counters?.momPersonnel, (raw2.momPersonnel ?? []).map((item) => item.id), defaults.counters.momPersonnel),
      testimonials: nextCounter(raw2.counters?.testimonials, (raw2.testimonials ?? []).map((item) => item.id), defaults.counters.testimonials),
      maids: nextCounter(raw2.counters?.maids, maids.map((item) => item.id), defaults.counters.maids),
      enquiries: nextCounter(raw2.counters?.enquiries, enquiries.map((item) => item.id), defaults.counters.enquiries),
      clients: nextCounter(raw2.counters?.clients, clients.map((item) => item.id), defaults.counters.clients),
      agencyAdmins: nextCounter(raw2.counters?.agencyAdmins, agencyAdmins.map((item) => item.id), defaults.counters.agencyAdmins),
      directSales: nextCounter(raw2.counters?.directSales, directSales.map((item) => item.id), defaults.counters.directSales),
      chatMessages: nextCounter(raw2.counters?.chatMessages, chatMessages.map((item) => item.id), defaults.counters.chatMessages)
    }
  };
}, "mergeAppData");
var loadData = /* @__PURE__ */ __name(async (kv) => {
  ...
  const merged = mergeAppData(JSON.parse(stripBom(raw2)));
  await kv.put("app-data.json", JSON.stringify(merged));
  return merged;
}, "loadData");
var saveData = /* @__PURE__ */ __name(async (kv, data) => {
...
var validateMaidPayload = /* @__PURE__ */ __name((maid) => {
  const missing = requiredMaidFields.filter((field) => maid[field] === void 0);
  if (missing.length > 0) {
...
var toMaidRecordPayload = /* @__PURE__ */ __name((maid) => {
  const photoDataUrl = typeof maid.photoDataUrl === "string" ? maid.photoDataUrl : "";
  const photoDataUrls = Array.isArray(maid.photoDataUrls) ? maid.photoDataUrls.filter((item) => typeof item === "string") : photoDataUrl ? [photoDataUrl] : [];
  return {
...
app.post("/api/client-auth/register", async (c) => {
  ...
  const existing = data.clients.find(
    (client2) => client2.email.toLowerCase() === body.email.trim().toLowerCase()
  );
  if (existing) {
...
app.post("/api/client-auth/login", async (c) => {
  ...
  const client = data.clients.find(
    (item) => item.email.toLowerCase() === body.email.trim().toLowerCase() && item.password === body.password.trim()
  );
  if (!client) {
...
app.post("/api/agency-auth/register", async (c) => {
  ...
  const existing = data.agencyAdmins.find(
    (admin2) => admin2.username.toLowerCase() === body.username.trim().toLowerCase()
  );
  if (existing) {
...
app.post("/api/agency-auth/login", async (c) => {
  ...
  const admin = data.agencyAdmins.find(
    (item) => item.username.toLowerCase() === body.username.trim().toLowerCase() && item.password === body.password.trim()
  );
  if (!admin) {
...
var path_default = {
  async fetch(request, env, executionContext) {
    ...
    if (!isAssetRequest) {
      ...
      return env.ASSETS.fetch(spaRequest2);
    }
